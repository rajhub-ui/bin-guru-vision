...
function ScanPage() {
  ...
  const reset = () => { setItems(null); setSummary(""); setPreviewUrl(null); };
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        >
          ...
          {previewUrl ? (
            <img src={previewUrl} alt="upload preview" className="max-h-[420px] rounded-xl soft-shadow" />
          ) : (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-xs px-2 py-0.5 rounded-full bg-accent text-accent-foreground">{d.label}</div>
                            </div>
                            <div className="mt-2 flex items-center gap-2">
                              <Progress value={Math.round(it.confidence * 100)} className="h-1.5" />
                              <span className="text-xs tabular-nums text-muted-foreground">{Math.round(it.confidence * 100)}%</span>
                            </div>
